...
# 駅ページが hazards の下に毎回出している注記（src/lib/dictionaries.ts の hazardNote）。
HAZARD_NOTE = (
    "浸水想定区域とは、想定しうる最大規模の雨や台風が起きた場合に浸水すると"
    "試算された範囲です。ふだんから浸水する場所という意味ではなく、また区域の外なら"
    "浸水しないという意味でもありません。想定される深さは同じ駅でも区画ごとに違うため、"
    "住む場所を決める前に、区が公開しているハザードマップで住所ごとに確認してください。"
)
...
BEFORE_AFTER = [
    ...
    ("目黒川の氾濫が想定されており、区の洪水ハザードマップでは川沿いの低い区画が浸水想定区域に含まれる",
     "目黒区の洪水ハザードマップでは、目黒川沿いの低い区画が浸水想定区域に含まれる。これは想定しうる最大規模の降雨を前提にした試算であり、その規模の雨が降れば目黒川があふれる可能性がある、という意味である"),
    ("川沿いの利便性と坂の上の安全性はトレードオフになる",
     "坂を上がった区画は想定される浸水の深さが小さくなるため、川沿いの便利さを取るか、浸水の想定が小さい坂の上を取るかという選択になる"),
    ("日比谷線の始発があり、六本木・虎ノ門方面へは座って通勤できる",
     "日比谷線の始発があるため、六本木・虎ノ門方面へは座って通勤できる。ただしピーク時はホームに列ができ、1本見送ることもある"),
    ("目黒川の谷地形で坂が続く点も、住み始めてから毎日の移動で負担になる",
     "目黒川の谷地形で坂が続く点も、住み始めてから毎日の移動で負担に感じる人がいる"),
    ("駅至近だが価格帯は高く、ここだけで買い物を済ませると食費が上がる",
     "駅至近だが価格帯は高く、ここだけで買い物を済ませると、平均的な収入の世帯には食費の負担が大きくなる"),
    ("この街の生活コストの低さを支えている",
...
def render(d):
    ...
    for key in ("hazards", "stationNote", "nightWalk", "rentReason",
        ...
        if key not in d:
            continue
        inner = f"<p>{e(d[key])}</p>"
        # 災害は駅ページでも同じ注記を毎回そえている（src/lib/dictionaries.ts の hazardNote）。
        if key == "hazards":
            inner += f'<p class="note-small">{e(HAZARD_NOTE)}</p>'
        parts.append(block(LABELS[key], inner))
...
<div class="wrap">
  ...
  <section class="ba">
    ...
    <p class="note">
      ...
      第七に、災害はハザードマップに書いてある事実として書き、
      「危険」「安全」の評価を加えないこと。
      第八に、始発だから座れる・坂だから負担になるといった、
      条件や人によって変わることを言い切らないこと。
      第九に、費用は「食費が上がる」で終わらせず、
      誰の家計にどう効くのかまで書くこと。
      該当した箇所を合計145以上書き直した。
    </p>
  </section>
  ...
  <section class="caveat">
    ...
    <p>
      ...
      2026-09-13 に、店名・坂・家賃・浸水想定・病院を実データから取り直した。
      店名と病院は OpenStreetMap（ODbL）、標高は国土地理院の標高API、
      浸水想定は重ねるハザードマップ、家賃は LIFULL HOME'S と Yahoo!不動産が
      公開する駅別の相場から取っている。それ以前は書き手の記憶で書いており、
      スーパー64件のうち50件が実在せず、坂は17駅中11駅で判定が違い、
      うち5駅は上りと下りが逆だった。
    </p>
  </section>
