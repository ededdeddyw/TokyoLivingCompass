...
# 前後に漢字・カタカナ・英字が続く場合は、より長い固有名詞の一部
# （「深川立川病院」「フジテレビ湾岸スタジオ」）とみなして対象から外す。
# 範囲の基準として使うときは「深川で」「深川の」のように前後にひらがなや句読点が来る。
NAME_CHAR = r"一-龥ァ-ヶーA-Za-z"
AREA_BARE = re.compile(
    r"(?<![" + NAME_CHAR + r"])"
    r"(" + "|".join(AREA_NAMES) + r")"
    r"(?!エリア|地区|一帯|方面|に住|めし|飯|風|[" + NAME_CHAR + r"])"
)
