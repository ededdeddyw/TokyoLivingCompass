...
data/
├── roster/
│   └── stations.json   # 掲載候補の全駅（458駅）。駅名・所在区・路線のみ
├── computed/
│   ├── commutes.json   # 計算した所要時間（458駅 × 7オフィス街）
...
- **ロースター**（`roster/stations.json`）は「対象になる駅の全体像」。458駅。
  詳細プロフィールの有無を問わず、駅名・所在区・路線を持つ。作り方は
  [09-station-roster.md](./09-station-roster.md)。
- **計算値**（`computed/`）は公開データから機械生成する層。全458駅ぶんある。
...
駅側は `lineIds: ["99302"]` のように id で参照する。458駅規模で路線名を各駅に
持たせると二重管理になるため。
